...
const postsTable = schemaTables['posts'];

export async function up(knex: Knex): Promise<any> {
	schemaTables.forEach(async (table) => {
		console.log(table);
		const objTable = Object.keys(table);
		Object.keys(objTable).forEach((columnName) => {
			console.log(columnName);
		});
		// await knex.schema.createTable(table, (table) => {
		// 	Object.keys(objTable).forEach((columnName) => {
		// 		const column = objTable[columnName];
		// 		const { type, ...options } = column;
		// 		//table[type](columnName, ...Object.values(options));
		// 	});
		// });
	});

	// await knex.schema.createTable('posts', (table) => {
	// 	Object.keys(postsTable).forEach((columnName) => {
	// 		console.log(postsTable[columnName]);
	// 		const column = postsTable[columnName];
	// 		const { type, ...options } = column;
	// 		table[type](columnName, ...Object.values(options));
	// 	});
	// });
