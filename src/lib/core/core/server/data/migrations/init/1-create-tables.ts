...
export async function up(knex: Knex): Promise<any> {
	await knex.schema.createTable('newsletters', (table) => {
		...
		table.enu('title_font_category', ['serif', 'sans_serif']).notNullable().defaultTo('sans_serif');
		...
		table.enu('body_font_category', ['serif', 'sans_serif']).notNullable().defaultTo('sans_serif');
	...
	knex.schema.createTable('posts_authors', (table) => {
		table.string('id', 24).notNullable().primary();
	...
	knex.schema.createTable('permissions_users', (table) => {
		table.string('id', 24).notNullable().primary();
